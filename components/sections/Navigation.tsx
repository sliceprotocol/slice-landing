...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const darkSectionIds = [
        "justice-stack", // JusticeFeatures - "Why Justlys Work for Real Products"
      ...
      darkSectionIds.forEach((id) => {
        ...
        if (element) {
          ...
          const isOverlapping =
            navbarViewportBottom > elementTop &&
            navbarViewportTop < elementBottom;
          const isWellInside =
            navbarViewportCenter >= elementTop + threshold &&
      ...
      if (!isOverDark) {
        const darkSections = document.querySelectorAll(
          'section[class*="bg-[#050505]"], section[class*="bg-[#080808]"], section[class*="bg-black"]',
        );
  ...
  return (
    <div className="fixed top-6 inset-x-0 z-50 flex flex-col items-center pointer-events-none px-6">
      <nav
        className={`pointer-events-auto w-full max-w-5xl flex items-center justify-between rounded-full border backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] pl-5 pr-2 py-2.5 transition-all duration-300 ${
          isDarkBackground
