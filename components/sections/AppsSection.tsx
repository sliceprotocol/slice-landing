            ...
            {type === "dots" && (
                <div className="grid grid-cols-3 gap-1">
                    {[...Array(9)].map((_, i) => (
                        <div
                            key={i}
                            className={`size-1.5 rounded-full ${i % 2 === 0 ? "bg-[#BC5FEF]" : "bg-gray-300"} ${i % 2 === 0 ? "opacity-100" : "opacity-40"}`}
                        />
                    ))}
                </div>
            )}
...
export function AppsSection() {
    return (
        <section id="integrations" className="relative py-24 lg:py-64 bg-[#F9FAFB] overflow-hidden">
            ...
            <div className="container relative z-20 mx-auto max-w-4xl px-6 text-center">
                ...
                <div className="flex flex-wrap items-center justify-center gap-4">
                    <Link href={SLICE_LINKS.DOCS} target="_blank" rel="noopener noreferrer">
                        <Button
                            variant="outline"
                            className="h-12 px-6 rounded-lg border-gray-200 bg-white text-black font-semibold hover:bg-gray-50 hover:border-[#BC5FEF]/30 transition-all w-full sm:w-auto"
                        >
                            Integrate Slice
                        </Button>
                    </Link>
                    <Link href={SLICE_LINKS.APP} target="_blank" rel="noopener noreferrer">
                        <Button className="h-12 px-6 rounded-lg bg-[#BC5FEF] text-white font-bold hover:bg-[#a54bd6] shadow-[0_4px_14px_rgba(188,95,239,0.3)] transition-transform hover:-translate-y-0.5 w-full sm:w-auto">
                            Start Judging
                        </Button>
                    </Link>
                </div>
